...
public class SampleTxCase {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException,
		...
		pool.addUTXO(new UTXO(tx1.getHash(), 1), tx1.getOutput(1));
		...
		//double spending 
		tx2.addInput(tx1.getHash(), 0);
		sig=sign(keyPair.getPrivate(), tx2.getRawDataToSign(1));
		tx2.addSignature(sig, 1);
		tx2.finalize();
		System.out.println("Double spending  " + txHandler.isValidTx(tx2));
		...
		keyPair2 = generateNewKeyPair();
		tx2.addOutput(1.0, keyPair2.getPublic());
		sig = sign(keyPair.getPrivate(), tx2.getRawDataToSign(1));
		tx2.addSignature(sig, 0);		
		sig = sign(keyPair.getPrivate(), tx2.getRawDataToSign(0));
		tx2.addSignature(sig, 1);		
		
		tx2.finalize();
		System.out.println("Wrong signature  " + txHandler.isValidTx(tx2));
		...
		tx2.addInput(tx1.getHash(), 0);
		keyPair2 = generateNewKeyPair();
		tx2.addOutput(11.0, keyPair2.getPublic());
		sig = sign(keyPair.getPrivate(), tx2.getRawDataToSign(0));
		tx2.addSignature(sig, 0);		
		tx2.finalize();
	
		System.out.println("output greater the input  " + txHandler.isValidTx(tx2));
		 ...
		 */
		// testing  handletxs
		System.out.println("Testing handleTxs");
		...
		sig = sign(keyPair.getPrivate(), tx3.getRawDataToSign(0));
		tx3.addSignature(sig, 0);		
		tx3.finalize();
		Transaction []arr= {tx2,tx3};
		System.out.println("Two Mutually valid transactions length = "+txHandler.handleTxs(arr).length);
